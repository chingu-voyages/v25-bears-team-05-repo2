...
describe("thread search tests", () => {
  test("gets correct public posts based on query (excludes threadComments)", async () => {
    ...
    await userDocuments[0].addConnectionToUser(
      userDocuments[1].id.toString(),
      true
    );
    ...
    await userDocuments[0].createAndPostThread({
      html: "one may fly over the cuckoo's nest",
      visibility: ThreadVisibility.Anyone,
    });
    await userDocuments[1].createAndPostThread({
      html: "why do they fly away? disney",
      visibility: ThreadVisibility.Anyone,
    });
    await userDocuments[1].createAndPostThread({
      html: "You and I were meant to fly",
      visibility: ThreadVisibility.Connections,
    });

    await userDocuments[0].createAndPostThread({
      html:
        "What an amazing story about an amazing person. This story talked about all the bad times and all the great accomplishments Walt Disney did in his wonderful life, there is a lot of wonderful information about him as a young person and how he came from nothing to becoming the king of Disneyland.",
    });
  ...
  test("gets correct private posts based on a query", async () => {
    ...
    await userDocuments[4].createAndPostThread({
      html: "Nothing is so necessary for a young man",
      visibility: ThreadVisibility.Connections,
    });

    await userDocuments[4].createAndPostThread({
      html: "There are more things in heaven and earth, Heratio,",
      visibility: ThreadVisibility.Connections,
    });
    await userDocuments[4].createAndPostThread({
    ...
    const result1 = await queryPrivateThreads({
      requestorUserId: userDocuments[0].id.toString(),
      queryString: "heratio",
    });
    const result2 = await queryPrivateThreads({
      requestorUserId: userDocuments[4].id.toString(),
      queryString: "young man",
    });
    ...
    expect(result1[0].content.html).toBe(
      "There are more things in heaven and earth, Heratio,"
    );
