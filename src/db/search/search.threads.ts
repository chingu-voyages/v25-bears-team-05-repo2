...
export async function queryPublicThreads(data: { queryString: string }): Promise<IThreadDetails[]> {
  ...
  const queryResults = await ThreadModel.find( { "$and" : [{ "$text": query }, { "visibility": ThreadVisibility.Anyone }]});
...
export async function queryPrivateThreads(data: { requestorUserId: string, queryString: string }): Promise<IThreadDetails[]> {
  ...
  if (connectionOfUsers && connectionOfUsers.length > 0) {
    const connectionOfIds = connectionOfUsers.map(user => user._id.toString());
    const query = { "$search": data.queryString };
    const queryResults = await ThreadModel.find( { "$and" : [{ "$text": query }, { "visibility": ThreadVisibility.Connections },
